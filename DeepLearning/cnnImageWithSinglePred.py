...
# Input shape 128 x 128 pixels, 3 for color channels
...
# make a separate folder called single prediction
# it is at the same level as the test_set and training_set
# the folder should contain the pictures

import numpy as np
from keras.preprocessing import image

# load the test image 
test_image = image.load_img('dataset/single_prediction/cat.jpg', target_size = (128, 128))
test_image = image.img_to_array(test_image)

# need to expand the dims to have 4 dimensions
test_image = np.expand_dims(test_image, axis = 0)
result = classifier.predict(test_image)

# this gets the mapping of the 0,1 for cats and dogs
